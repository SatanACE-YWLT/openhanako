...
export function createMemoryTicker(opts) {
  ...
  function _observationUsage(usage, resolvedModel, latencyMs) {
    return {
      model: String(resolvedModel?.model?.id || resolvedModel?.model || ""),
  ...
  async function _runCacheSnapshotReflection({
    sessionPath,
    sessionId,
    messages,
    resolvedModel,
    ...
    try {
      ...
      if (typeof summaryManager.createRollingSummaryDraft !== "function") {
        throw new Error("summaryManager.createRollingSummaryDraft is required for cache snapshot reflection");
      }

      const draft = await summaryManager.createRollingSummaryDraft(sessionId, messages, resolvedModel, {
        ...rollingOptions,
        returnUsage: true,
        usageTrigger: trigger,
      });

      if (mode === "write" && draft?.data) {
        summaryManager.saveSummary(sessionId, draft.data);
      }
      ...
      const observation = writeCacheSnapshotObservation(agentDir, {
        ...
        status: "success",
        reason: draft?.reason || "",
        usage: _observationUsage(draft?.usage, resolvedModel, Date.now() - startedAt),
        summaryPreview: _textPreview(draft?.summary || ""),
        memoryMdPreview: baseMemoryMd,
        baseMemoryMdHash: _sha256(baseMemoryMd),
        cachePrefixHash: draft?.cachePrefixHash || "",
      });
  ...
  async function _doRollingSummary(sessionPath, trigger = "threshold") {
    ...
    try {
      ...
      if (cacheSnapshotMode === "write") {
        await _runCacheSnapshotReflection({
          sessionPath,
          sessionId,
          messages,
          resolvedModel,
